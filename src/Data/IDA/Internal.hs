...
-- Copyright   :  Peter Robinson 2014-2020
-- License     :  LGPL
--
...
encode m numFragments msg
  | numFragments < 1 || numFragments >= 1021  =
  ...
  let (intseq,trailLen) = toIntVec m msg
      ...
      c i k = dotProduct (M.getRow i vm) (blocks ! (k-1))
      content i = [ c i j
...
decode pss@(p:_)
  | length pss < reconstructionThreshold p =
...
-- 'Vector Word8', appending 0s at the end such that the length is divisible by
-- m.
